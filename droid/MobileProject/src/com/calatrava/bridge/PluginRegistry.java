...
public class PluginRegistry {
	...
	public PluginRegistry(String packageName, Context appContext,
			RhinoService rhino) throws IOException, URISyntaxException,
	...
	private void addPlugins(String packageName, Context context)
		...
		AnnotationRegistrar registrar = new AnnotationRegistrar(packageName,
				context, "com.calatrava.bridge");
		registrar.register(new Registration() {
			public void install(Annotation annotation, Class<?> toRegister) {
				if (annotation instanceof CalatravaPlugin) {
					...
					try {
						RegisteredPlugin plugin = (RegisteredPlugin) toRegister
								.newInstance();
						plugin.setContext(PluginRegistry.this, appContext);
						registeredPlugins.put(pluginName, plugin);
					} catch (Exception e) {
						Log.e(TAG, "Unable to instantiate plugin: "
								+ pluginName, e);
					}
				}
			}
	...
	public void runCommand(Intent intent, RegisteredActivity frontmost) {
		installedCmds.get(intent.getExtras().getString("command")).execute(
				intent, frontmost);
	}
	...
	public void call(String plugin, String method, String argsJson) {
		try {
			if (registeredPlugins.get(plugin) != null) {
				registeredPlugins.get(plugin).call(method,
	...
	public void invokeCallback(String callbackHandle, Object data) {
		rhino.callJsFunction("calatrava.inbound.invokePluginCallback",
				new String[] { callbackHandle, data.toString() });
	}
}
